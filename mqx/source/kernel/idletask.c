...
{ /* Body */
    ...
    while (1) {
        ...
        /*
         * Everytime idletask is hit and m4 status indicate low power state,
         * M4 will went into TCM and inform A9 it is in "LPRUN" state.
         * Everytime a interrupt wakeup M4 from it, it will tell A9
         * of "HIRUN" state, waiting A9 response. When comes out of the
         * wfi state, normally the interrupt will be called and normal
         * tasks will be excecuted first, then idle task can be run again
         *
         * The idletask also need to check A9's status to determine if TCM
         * WFI is needed. If A9 is not ready, normal WFI is enough for
         * power saving, the logic are all in "_io_mcore_lpm_wfi"
         */
        _io_mcore_lpm_wfi();
#endif
    } /* Endwhile */
