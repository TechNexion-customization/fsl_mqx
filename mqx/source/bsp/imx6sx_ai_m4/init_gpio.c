...
/*FUNCTION*-------------------------------------------------------------------
*
* Function Name    : _bsp_can_transceiver_wake_up
* Returned Value   : MQX_OK for success, -1 for failure
* Comments         :
*    This function performs BSP-specific initialization related to CAN transceiver
*
*END*----------------------------------------------------------------------*/

void _bsp_can_transceiver_wake_up(void)
{
    MQX_FILE_PTR fd;
    _mqx_int param;

    // Enable ii2c driver
    fd = _io_fopen("ii2c3:", NULL);

    param = 100000;
    _io_ioctl(fd, IO_IOCTL_I2C_SET_BAUD, &param);
    _io_ioctl(fd, IO_IOCTL_I2C_SET_MASTER_MODE, NULL);
    param = 0x60;
    _io_ioctl(fd, IO_IOCTL_I2C_SET_STATION_ADDRESS, &param);
    param = 0x32;
    _io_ioctl(fd, IO_IOCTL_I2C_SET_DESTINATION_ADDRESS, &param);

    // Set can wake-up pin as output
    param = 0x03;
    _io_write(fd, &param, 1);
    param = 0x7F;
    _io_write(fd, &param, 1);
    _io_fflush(fd);
    _io_ioctl(fd, IO_IOCTL_I2C_STOP, NULL);
