...
void *_bsp_get_ecspi_base_address(uint32_t channel)
{
    ...
    if (channel > 0 && channel <= ELEMENTS_OF(ecspi_address)) {
        return (void *)ecspi_address[channel - 1];
    }
    return NULL;
}
...
{
    /* channel start from 1 */
    if (channel > 0 && channel <= ELEMENTS_OF(ecspi_vectors)) {
        return ecspi_vectors[channel - 1];
    }
    return 0;
}
