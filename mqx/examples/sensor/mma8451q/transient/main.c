...
#error This application requires BSPCFG_ENABLE_IO_SUBSYSTEM defined non-zero in user_config.h. Please recompile BSP with this option.
        ...
        #error This application requires BSPCFG_ENABLE_I2C3 or BSPCFG_ENABLE_II2C3 defined non-zero in user_config.h. Please recompile BSP with this option.
            ...
            #define MMA8451Q_INT_ROUTE      MMA8451Q_TRANS_INT_ROUTE_TO_INT2
            ...
            #define MMA8451Q_INT_ROUTE      MMA8451Q_TRANS_INT_ROUTE_TO_INT1
    #endif
#endif
...
void main_task(uint32_t dummy)
{
    ...
    MMA8451Q_INIT_STRUCT    mma8451q_init_str = {
                                                   ...
                                                   .OUTPUT_DATA_RATE     = MMA8451Q_OUTPUT_DATA_RATE_100HZ,
    ...
    printf("\n\n-------------- MMA8451Q sensor driver transient example --------------\n\n");
    ...
    printf("Set_transient_bypass_hpf... ");
    if(mma8451q_set_transient_bypass_hpf(mma8451q_handle, MMA8451Q_TRANSIENT_CFG_HPF_ENABLE))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_transient_event_latch_state... ");
    if(mma8451q_set_transient_event_latch_state(mma8451q_handle, MMA8451Q_TRANSIENT_CFG_ELE_ENABLE))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_transient_threshold... ");
    if(mma8451q_set_transient_threshold(mma8451q_handle, 0x08))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_transient_db_cnt_mode... ");
    if(mma8451q_set_transient_db_cnt_mode(mma8451q_handle, MMA8451Q_TRANSIENT_CFG_DBCNTM_DECREMENT))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_transient_db_cnt... ");
    if(mma8451q_set_transient_db_cnt(mma8451q_handle, 1))
        {
        printf("OK\n");
    }
    else
    ...
    printf("Set_transient_state... ");
    if(mma8451q_set_transient_state(mma8451q_handle, MMA8451Q_TRANSIENT_ENABLE_X | MMA8451Q_TRANSIENT_ENABLE_Y))
    {
        printf("OK\n");
    }
    else
    ...
    ******************************************************************************/
    ...
    printf("Route transient interrupt to Int pin...\n");
    ...
    printf("Enable transient interrupt ...\n");
    if (!mma8451q_set_int_state(mma8451q_handle, MMA8451Q_TRANS_INT_ENABLE))
    {
        printf("ERROR\n");
        _task_block();
    }
    ...
    printf("\nShake the board on table to trigger transient detection on X/Y axis\n");
    ...
    for (i = 0; i < 50; i++)
    {
        ...
        if (buffer[0] == MMA8451Q_INT_SOURCE_TRANS)
        {
            /* Read ff_mt status to clean transient interrupt flag */
            if(mma8451q_get_transient_status(mma8451q_handle, buffer))
            {
                printf("Transient detected ");
                if(*buffer & MMA8451Q_TRANSIENT_EVENT_Z_DETECT)
                {
                    printf("on:Z-AXIS\n");
                }
                else if(*buffer & MMA8451Q_TRANSIENT_EVENT_Y_DETECT)
                {
                    printf("on:Y-AXIS\n");
                }
                else if(*buffer & MMA8451Q_TRANSIENT_EVENT_X_DETECT)
                {
                    printf("on:X-AXIS\n");
                }
                else
