...
#error This application requires BSPCFG_ENABLE_IO_SUBSYSTEM defined non-zero in user_config.h. Please recompile BSP with this option.
        ...
        #error This application requires BSPCFG_ENABLE_I2C3 or BSPCFG_ENABLE_II2C3 defined non-zero in user_config.h. Please recompile BSP with this option.
            ...
            #define MMA8451Q_INT_ROUTE      MMA8451Q_LNDPRT_INT_ROUTE_TO_INT2 | MMA8451Q_ASLP_INT_ROUTE_TO_INT2
            ...
            #define MMA8451Q_INT_ROUTE      MMA8451Q_LNDPRT_INT_ROUTE_TO_INT1 | MMA8451Q_ASLP_INT_ROUTE_TO_INT1
    #endif
#endif
...
void main_task(uint32_t dummy)
{
    ...
    MMA8451Q_INIT_STRUCT    mma8451q_init_str = {
                                                   ...
                                                   .ACTIVE_POWER_SCHEME  = MMA8451Q_ACTIVE_POWER_SCHEME_HIGH_RESOLUTION,
                                                   .BURST_READ_MODE      = MMA8451Q_BURST_READ_MODE_NORMAL,
                                                };

    printf("\n\n-------------- MMA8451Q sensor driver low power example --------------\n\n");
    ...
    printf("Set_aslp_state... ");
    if (mma8451q_set_aslp_state(mma8451q_handle, MMA8451Q_AUTO_SLEEP_ENABLE))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_aslp_output_data_rate... ");
    if (mma8451q_set_aslp_output_data_rate(mma8451q_handle, MMA8451Q_ASLP_OUTPUT_DATA_RATE_6HZ25))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_aslp_power_scheme... ");
    if (mma8451q_set_aslp_power_scheme(mma8451q_handle, MMA8451Q_ASLP_POWER_SCHEME_LOW_POWER))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_wake_up_bypass... ");
    if (mma8451q_set_wake_up_bypass(mma8451q_handle, MMA8451Q_ASLP_WAKE_LNDPRT_ENABLE))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_aslp_count... ");
    if (mma8451q_set_aslp_count(mma8451q_handle, 5))
    {
        printf("OK\n");
    }
    else
    ...
    ******************************************************************************/
    ...
    if (!mma8451q_set_int_state(mma8451q_handle, MMA8451Q_LNDPRT_INT_ENABLE | MMA8451Q_ASLP_INT_ENABLE))
    {
        printf("ERROR\n");
        _task_block();
    }
    ...
    printf("\nKeep the board orientation & wait the sensor fall asleep \n");
    printf("Change the board orientation to wake the board from sleep \n\n");
    ...
    while (1)
    {
        ...
        buffer[0] = 0x00;
        mma8451q_get_int_source(mma8451q_handle, &int_source);
        if (int_source & MMA8451Q_INT_SOURCE_LNDPRT)
        {
            /* Read lapo status to clean lapo interrupt flag */
        ...
        if (int_source & MMA8451Q_INT_SOURCE_ASLP)
        {
            if (mma8451q_get_system_mode(mma8451q_handle, buffer))
            {
                if (MMA8451Q_SYSMOD_WAKE == *buffer)
                {
                    printf("orientation changed\n");
                    printf("MMA8451Q wake up\n");
                    if (i == 9)
                        break;
                    i++;
                }
                else if(MMA8451Q_SYSMOD_SLEEP == *buffer)
                {
                    printf("MMA8451Q fall asleep\n");
                }
                else
                {
                    printf("ERROR\n");
