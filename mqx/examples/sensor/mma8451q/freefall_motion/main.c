...
#error This application requires BSPCFG_ENABLE_IO_SUBSYSTEM defined non-zero in user_config.h. Please recompile BSP with this option.
        ...
        #error This application requires BSPCFG_ENABLE_I2C3 or BSPCFG_ENABLE_II2C3 defined non-zero in user_config.h. Please recompile BSP with this option.
            ...
            #define MMA8451Q_INT_ROUTE      MMA8451Q_FF_MT_INT_ROUTE_TO_INT2
            ...
            #define MMA8451Q_INT_ROUTE      MMA8451Q_FF_MT_INT_ROUTE_TO_INT1
    #endif
#endif
...
void main_task(uint32_t dummy)
{
    ...
    printf("\n\n-------------- MMA8451Q sensor driver freefall & motion example --------------\n\n");
    ...
    printf("Set_ff_mt_selection... ");
    if(mma8451q_set_ff_mt_selection(mma8451q_handle, MMA8451Q_FF_MT_SELECT_MOTION))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_ff_mt_event_latch_state... ");
    if(mma8451q_set_ff_mt_event_latch_state(mma8451q_handle, MMA8451Q_FF_MT_CFG_ELE_ENABLE))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_ff_mt_threshold... ");
    if(mma8451q_set_ff_mt_threshold(mma8451q_handle, 3))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_ff_mt_db_cnt_mode... ");
    if(mma8451q_set_ff_mt_db_cnt_mode(mma8451q_handle, MMA8451Q_FF_MT_CFG_DBCNTM_DECREMENT))
    {
        printf("OK\n");
    }
    else
    ...
    printf("Set_ff_mt_db_cnt... ");
    if(mma8451q_set_ff_mt_db_cnt(mma8451q_handle, 1))
        {
        printf("OK\n");
    }
    else
    ...
    printf("Set_ff_mt_state... ");
    if(mma8451q_set_ff_mt_state(mma8451q_handle, MMA8451Q_FF_MT_ENABLE_X | MMA8451Q_FF_MT_ENABLE_Y))
    {
        printf("OK\n");
    }
    else
    ...
    ******************************************************************************/
    ...
    printf("Route Freefall & motion interrupt to Int pin...\n");
    ...
    printf("Enable FF_MT interrupt ...\n");
    if (!mma8451q_set_int_state(mma8451q_handle, MMA8451Q_FF_MT_INT_ENABLE))
    {
        printf("ERROR\n");
        _task_block();
    }
    ...
    printf("Move the board on table to trigger motion detection on X/Y axis\n");
    ...
    for (i = 0; i < 50; i++)
    {
        ...
        if (buffer[0] == MMA8451Q_INT_SOURCE_FF_MT)
        {
            /* Read ff_mt status to clean ff_mt interrupt flag */
            if(mma8451q_get_ff_mt_status(mma8451q_handle, buffer))
            {
                printf("Motion detected ");
                if(*buffer & MMA8451Q_FF_MT_EVENT_Z_DETECT)
                {
                    printf("on:Z-AXIS\n");
                }
                else if(*buffer & MMA8451Q_FF_MT_EVENT_Y_DETECT)
                {
                    printf("on:Y-AXIS\n");
                }
                else if(*buffer & MMA8451Q_FF_MT_EVENT_X_DETECT)
                {
                    printf("on:X-AXIS\n");
                }
                else
