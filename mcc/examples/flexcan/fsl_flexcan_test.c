...
void Main_Task(uint32_t parameter)
{
     ...
     * Wait For CAN Peer Become Ready
     */
    printf("\n\n\n\n***** MCC FLEXCAN EXAMPLE *****\n");
    printf("Please wait :\n");
    printf("    1) A9 peer is ready\n");
    printf("    2) CAN peer is ready\n");
...
#if defined(BSP_IMX6SX_PELE_M4) || defined(BSP_IMX6SX_SDB_M4) || defined(BSP_VYBRID_AUTOEVB_M4) ||\
    defined(BSP_TWR_VF65GS10_A5) || defined(BSP_IMX6SX_AI_M4)
    flexcan1_data.num_rximr = 64;
#else
    flexcan1_data.num_rximr = 16;
#endif
    ...
    result = flexcan_init(instance, &flexcan1_data, TRUE);
    if (result)
...
MCC_ENDPOINT    mqx_endpoint_a5 = {0,MCC_MQX_NODE_A5,MCC_MQX_CAN_A5_PORT};
MCC_ENDPOINT    mqx_endpoint_m4 = {1,MCC_MQX_NODE_M4,MCC_MQX_CAN_M4_PORT};
...
void Rx_Task(uint32_t parameter)
{
    ...
    ret_value = mcc_get_info(MCC_MQX_NODE_M4, &mcc_info);
    if(MCC_SUCCESS == ret_value && (strcmp(mcc_info.version_string, MCC_VERSION_STRING) != 0)) {
        ...
        mcc_destroy(MCC_MQX_NODE_M4);
        _task_block();
    }
    printf("\n\n\nMCC CAN M4 Side Started, MCC version is %s\n", mcc_info.version_string);
    ...
    while(1)
    {
        ...
        else
        {
            if (is_rx_mb_data)
            {
                ...
                // MCC Part
                sprintf(msg.DATA, "\nDLC=%d, mb_idx=%d", temp, RX_mailbox_num);
                mcc_send(&mqx_endpoint_m4, &mqx_endpoint_a5, &msg, sizeof(THE_MESSAGE), 0xffffffff);
                sprintf(msg.DATA, "\nRX MB data: 0x");
                mcc_send(&mqx_endpoint_m4, &mqx_endpoint_a5, &msg, sizeof(THE_MESSAGE), 0xffffffff);
                ...
                for (result = 0; result < temp; result++) {
                    ...
                    // MCC Part
                    sprintf(msg.DATA, "%02x ", rx_mb.data[result]);
                    mcc_send(&mqx_endpoint_m4, &mqx_endpoint_a5, &msg, sizeof(THE_MESSAGE), 0xffffffff);
                }
            ...
            if (is_rx_fifo_data)
            {
                ...
                for (result = 0; result < temp; result++) {
                    ...
                    // MCC Part
                    sprintf(msg.DATA, "%02x ", rx_fifo.data[result]);
                    mcc_send(&mqx_endpoint_m4, &mqx_endpoint_a5, &msg, sizeof(THE_MESSAGE), 0xffffffff);
                }
